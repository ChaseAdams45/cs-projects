...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if isinstance(insect, QueenAnt):
            if not insect.real_queen_implemented:
                return None
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant == None and not isinstance(self, ContainerAnt):
                place.ant.contain_ant(self)
            elif isinstance(self, ContainerAnt) and not isinstance(place.ant, ContainerAnt) and self.contained_ant == None:
                place.ant, self.contained_ant = self, place.ant
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        position = 0
        while place != beehive:
            if place.bees and position >= self.min_range and position <= self.max_range:
...
class BodyguardAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        if self.contained_ant:
            self.contained_ant.action(gamestate)  # Change to True to view in the GUI
    # END Problem 9
...
# END Problem 13
   ...
   # Change to True to view in the GUI
    ...
    def __init__(self, armor=1):
        ...
        self.real_queen_implemented = QueenAnt.real_queen_implemented
        QueenAnt.real_queen_implemented = True
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 13
        if self.real_queen_implemented:
            self.reduce_armor(self.armor)
        else:
            ScubaThrower.action(self, gamestate)
            position = self.place.exit
            while position:
                if isinstance(position.ant, ContainerAnt) and position.ant.contained_ant and position.ant.contained_ant not in self.doubled_ants:
                    position.ant.contained_ant.damage *= 2
                    self.doubled_ants.append(position.ant.contained_ant)
                if position.ant and position.ant not in self.doubled_ants:
                    position.ant.damage *= 2
                    self.doubled_ants.append(position.ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.real_queen_implemented:
            ...
            self.armor -= amount
            if self.armor <= 0 :
                if not  self.real_queen_implemented:
                    bees_win()
